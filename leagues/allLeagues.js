  ...
  const getAllLeagues = async () => {
    ...
    const popular = result.popular.map((pop) => {
      return {
        ...
        logo: {
          light: `https://api.soccerhub.pro/image_resources/logo/leaguelogo/light/${pop.id}.png`,
          dark: `https://api.soccerhub.pro/image_resources/logo/leaguelogo/dark/${pop.id}.png`,
        },
      };
    });
    ...
    const international = result.international.map((int) => {
      const leagues = int.leagues.map((league) => {
        return {
          ...
          logo: {
            light: `https://api.soccerhub.pro/image_resources/logo/leaguelogo/light/${league.id}.png`,
            dark: `https://api.soccerhub.pro/image_resources/logo/leaguelogo/dark/${league.id}.png`,
          },
        };
      });
